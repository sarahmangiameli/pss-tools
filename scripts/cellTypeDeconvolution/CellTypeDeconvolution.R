...
brCounts <- fread("./PublicData/GSE123576_mousebrain_countsData_revision.csv.gz")
brPeaks <- fread("./PublicData/GSE123576_mousebrain_peaks_revision.bed.gz")

sparseCounts <- sparseMatrix(as.integer(brCounts$peak_idx), as.integer(brCounts$cell_idx), x = brCounts$count)
saveRDS(sparseCounts, "sparseCounts.rds")
...
rownames(bioRadBrainCounts) <- paste(as.character(seqnames(peak_ranges)), 
                                     ...
                                     sep = ":")
saveRDS(bioRadBrainCounts, "bioRadBrainCounts.rds")
...
bioRadCisTopicObject <- runWarpLDAModels(bioRadCisTopicObject, topic=c(200),
                                         seed=2022, nCores=1, iterations = 500, addModels=FALSE)
saveRDS(bioRadCisTopicObject, "bioRadCisTopicObject.rds")
...
bioRadBrainMeta <- fread("./PublicData/GSE123576_mousebrain_cellData_revision.tsv.gz")
...
brGitHubIdents <- fread("./PublicData/revision-cluster_annotations.tsv")
...
saveRDS(umap.d, "umap.d.rds")

# Plot UMAP
pdf("bioRadMouseBrainCisTopic200Umap.pdf")
...
saveRDS(dev_custom, "deviationsCisTopicOligGranBioRad.rds")
...
pdf("CorrelationHeatmapPlusPublic.pdf")
...
pdf("barPlotCellTypesGrouped.pdf")
...
pdf("barPlotCellTypesGroupedWithPublic.pdf")
...
pdf("barPlotCellTypesGroupedWithMixed.pdf")
